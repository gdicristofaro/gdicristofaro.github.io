...
const PhoneIcon = () => (
  <svg
    className="highlight-color icon inline-block align-text-bottom mr-1/2"
    fill="none"
    strokeWidth="1.5"
    stroke="currentColor"
    viewBox="0 0 24 24"
...
const LinkIcon = () => (
  <svg
    className="highlight-color icon inline-block align-baseline mr-1/2"
    fill="none"
    strokeWidth="1.5"
    stroke="currentColor"
    viewBox="0 0 24 24"
...
const CalendarIcon = () => (
  <svg
    className="icon inline-block align-baseline mr-1/2"
    fill="none"
    strokeWidth="1.5"
    stroke="currentColor"
    viewBox="0 0 24 24"
  ...
  return (
    <div className="resume-container">
      <header className="mb-3 break-inside-avoid-page">
        <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-100 mb-1 uppercase">
          {name}
        </h1>
      ...
      <div className="column-container sm:flex">
        <div
          className={`left-column mb-${sectionMargin} sm:mb-0 sm:${leftColumn} sm:pr-5`}
        >
          <section
            className={`summary mb-${sectionMargin} break-inside-avoid-page`}
          >
          ...
          <section className={`mb-${sectionMargin}`}>
            ...
            {workExperience.map((job, i) => (
              <div
                key={i}
                className={`mb-${subsectionMargin} break-inside-avoid-page`}
              >
                <p className="font-bold text-lg text-gray-800 dark:text-gray-100">
                  {job.title}
                </p>
                <p className="font-bold text-base mb-1 highlight-color">
                  {job.company}
                </p>
          ...
          <section className="break-inside-avoid-page">
            ...
            {education.map((edu, i) => (
              <div
                key={i}
                className={`${i < education.length - 1 ? `mb-${subsectionMargin}` : ""} ${i > 0 ? `pt-${subsectionMargin - 1} border-t border-dotted border-neutral-400` : ""}`}
              >
                <p className="font-bold text-lg text-gray-800 dark:text-gray-100">
                  {edu.degree}
                </p>
                <p className="font-bold mb-1 text-base highlight-color">
                  {edu.institution}
                </p>
        ...
        <div className={`right-column sm:${rightColumn}`}>
          ...
          <section className={`mb-${sectionMargin} break-inside-avoid-page`}>
            ...
            {achievements.map((achievement, i) => (
              <div
                key={i}
                className={`mb-3 text-sm${i > 0 ? " pt-3 border-t border-dotted border-neutral-400" : ""}`}
              >
          ...
          <section className="projects">
            ...
            {projects.map((project, i) => (
              <div
                key={i}
                className={`text-sm break-inside-avoid-page${i < projects.length - 1 ? " mb-3 pb-3 border-b border-dotted border-neutral-400" + (projectNoPrintBorderIdx.includes(i) ? " print-border-none" : "") : " mb-4"}`}
              >
                ...
                {project.link && (
                  <a className="highlight-color break-all" href={project.link}>
                    {project.link.replaceAll(/^https?:\/\/(www)?/gi, "")}
                  </a>
                )}
              </div>
            ))}
